...
import { ChatDTO } from '../../../features/type';
...
const formatChat = (chat: ChatDTO) => ({
    id: chat.id,
    // avatar: chat.avatar
    //     ? constants.GET_PHOTO + chat.avatar
    //     : chat.last_message?.user?.avatar
    //       ? constants.GET_PHOTO + chat.last_message.user.avatar
    //       : avatar,
    title: chat.title,
...
const getActiveChatId = (): null | number =>
    Store.getState().activeChatId as null | number;
...
const getChats = async () => {
    try {
        const responce = getStatus(await chatApi.get());
        if (responce.data) {
...
const createChat = async (title: string) => {
    try {
        const responce = getStatus(await chatApi.create({ title }));
        if (responce.error) {
            Store.set(
                ErrorsChats.errorCreateChat,
                `Ошибка создание чата: ${responce.error.reason}`,
            );
            return false;
        }
        getChats();
...
const deleteChat = async () => {
    try {
        ...
        const responce = getStatus(await chatApi.delete({ chatId: id }));
        if (responce.error) {
            Store.set(
                ErrorsChats.errorDeleteChat,
                `Ошибка удаление чата: ${responce.error.reason}`,
            );
            return false;
        }
        getChats();
    ...
    try {
        ...
        if (responceSearch.data && responceSearch.data[0]) {
            await chatApi.addUser({
                users: [responceSearch.data[0].id],
                chatId: id,
            });
        }
    } catch (error) {
...
const deleteUserChat = async (login) => {
    try {
        const id = getActiveChatId();
        if (!id) {
            Store.set(ErrorsChats.errorDeleteChat, 'Нет активного чата');
...
const changeChatAvatar = async (file) => {
    try {
        const id = getActiveChatId();
        if (!id) {
            Store.set(ErrorsChats.errorDeleteChat, 'Нет активного чата');
        ...
        const responce = await chatApi.updateAvatar({
            avatar: file,
            chatId: id,
        });
        if (responce.data) {
            ...
            const chatIndex = findActiveChatIndex(id);
            if (chatIndex !== -1) {
                listChat[chatIndex].avatar =
                    constants.GET_PHOTO + responce.data.avatar;
